...
--
-- Does NOT: drop/alter any RLS policy, rewrite data, change any table grant, revoke
-- any access to the TABLES, or touch migration 072 / any existing function. Direct
-- table access is intentionally NOT revoked here — that is a later, separate migration
-- once every read is proven cut over. (The direct-table RLS remains the backstop.)
--
-- Grants: this migration hardens its OWN new functions' ACLs up front (074 pattern) —
-- REVOKE ALL FROM PUBLIC + anon on every function, then GRANT EXECUTE only to
-- authenticated + service_role (participant-safe count/lobby included; anon stays
-- denied — Ralli demo is in-memory, no anon path needs them). The internal authz
-- helper is granted to no client role. Function bodies additionally enforce
-- role/tenant/session authorization on top of the grants.
-- ─────────────────────────────────────────────────────────────────────────────

-- Authorization predicate: may the CURRENT caller read MANAGER-scoped data for a
-- session hosted by p_host_id in tenant p_tenant? True iff the caller is:
--   • the EXACT session host (independently valid even if the host's role is 'user'),
--   • a SAME-TENANT tenant manager — profile role 'orgAdmin' OR 'manager' (both are
--     tenant management roles; managers are authorized for Ralli Live games/results
--     exactly like orgAdmin per src/lib/permissions.js), or
--   • a ralli_admin (platform operator; tenant-agnostic).
-- Managers/orgAdmins are strictly SAME-TENANT — never cross-tenant. auth.uid()
...
AS $$
  SELECT auth.uid() IS NOT NULL AND (
    ...
    OR EXISTS (
      ...
      WHERE p.id = auth.uid()
        AND ( (p.role IN ('orgAdmin','manager') AND p.tenant_id IS NOT NULL AND p.tenant_id::text = p_tenant)
              OR p.role = 'ralli_admin' )
    )
  );
$$;
COMMENT ON FUNCTION public.ralli_can_manage_session(text, text) IS
  'Ralli Live: true if the current caller may read manager-scoped data for a session (exact host, same-tenant orgAdmin OR manager, or ralli_admin). Server-side authz for the 075 read RPCs.';
...
BEGIN
  ...
  IF    v_role IN ('orgAdmin','manager') AND v_tenant IS NOT NULL THEN v_target := v_tenant::text;   -- own tenant (orgAdmin OR manager)
...
-- ── Grant hardening (grant-level parity from creation; the 074 pattern, applied up-front) ──
-- Strip the Postgres/Supabase schema-default EXECUTE (PUBLIC + the explicit anon
-- default-privilege grant) from every new function, then grant EXECUTE only where
-- required. anon is denied at the GRANT level (no proven demo path needs it — Ralli
-- demo is in-memory). Function bodies still enforce role/tenant/session authz on top.
REVOKE ALL ON FUNCTION public.ralli_can_manage_session(text, text)       FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_host_session_restore(uuid)             FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_manager_active_sessions(uuid)          FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_manager_session_history(uuid, integer) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_manager_session_analytics(uuid)        FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_session_player_counts(uuid[])          FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.rpc_lobby_participants(uuid)               FROM PUBLIC, anon;

-- Management RPCs: authenticated (body enforces host/manager/orgAdmin/ralli_admin) + service_role.
...
-- Participant-safe RPCs: authenticated (body allows manager OR participant) + service_role. anon stays denied.
...
-- Internal authz helper: NOT broadly executable. It is only ever called from the
-- SECURITY DEFINER RPCs above (which run as the function owner and can reach it
-- regardless of grants), so it is granted to no client role — REVOKE-only above.
