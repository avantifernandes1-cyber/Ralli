...
// verify-game-session` bundles the relative import below (../../../src/lib/
// gameGrading.js) via the eszip dependency-graph bundler. import_map.json in this
// folder pins the supabase-js version; the shared grader is a plain local ESM
...
Deno.serve(async (req) => {
  try {
    ...
    if (!sessionId || typeof sessionId !== "string") return json({ error: "session_id required" }, 400);
    ...
    const { data: session, error: sErr } = await admin
      ...
      .eq("id", sessionId)
      .maybeSingle();
    if (sErr) return json({ error: "session load failed" }, 500); // retryable
    ...
    if (pErr) return json({ error: "profile load failed" }, 500); // retryable
    if (!profile) return json({ error: "not authorized" }, 403);
    ...
    if (!authorized) return json({ error: "not authorized for this session" }, 403);

    // 5. Only real, durably-completed sessions are verifiable.
    if (session.demo_mode === true) return json({ status: "ineligible", reason: "demo_session" }, 200);
    if (session.status !== "completed") return json({ error: "session not completed", retryable: true }, 409);
    ...
    if (rErr) return json({ error: "verification write failed", retryable: true }, 500);
